...
class StrawberryInputForm extends Component {
    constructor(props) {
        super(props);
        this.state={
            name: '',
    ...
    submitFormHandler = (event) => {
        ...
        const form = {
            userName: name,
            userEmail: email,
            strawberryItems: this.props.StrawberryItems
        }
        axiosInstance.post("/order/strawberry.json", form)
        .then(response => {
...
const mapStateToProps = state => {
    return {
        StrawberryItems: state.strawberryItems
    }
}

export default connect(mapStateToProps)(StrawberryInputForm); 