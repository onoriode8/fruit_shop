...
//   const [edit, setEdit] = useState(false);
  ...
  const deleteAppleHandler = (e) => {
    e.preventDefault();
    axiosInstance.delete(`/order/apple/${props.appleId}.json`)
      .then(res => {
        setShowDelete(true)
        setTimeout(() => {
  ...
  return (
      <>
        {editApple ? <div style={{background: "rgba(0, 0, 0, 0.5)",
         width: "100%", height: "100vh", position: "fixed", top: "0"}}></div> : null }
        {editApple && <div><AppleUpdate appleId={props.appleId}/></div>}
        <header style={{display: "inline-block", margin: "5px 0px",  marginTop: "5em"}}>
            <div style={{textAlign: "center", margin: "0px 10px", boxShadow: "1px 2px 3px 4px #1134", padding: "30px 15px"}}>
                ...
                <img style={{width: "95px"}} src={props.appleImage} alt="" />
                <p>{props.appleproductName}</p>
                <p><strong>#{props.applePrice}</strong></p>
                <div style={{display: "flex", justifyContent: "space-evenly"}}>
                    <button style={{fontFamily: "Roman", 
                      fontSize: "10px", margin: "0px 4px"}} onClick={() => setEditApple(true)}>
